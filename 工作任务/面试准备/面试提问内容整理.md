...
### 项目背景
> 亚信这段项目的背景，是公司内部要自研 Agent 能力，主要解决 Agent 推理不可控、幻觉、上下文遗忘这些模型通病。我们参考了美国军工企业 Pylinuter 的思路，做了本体知识图谱，把企业知识结构化起来，尽量让推理链路可控、可追踪。

### 我的角色
> 我主导了多 Agent 协同工作流、状态管理和数据库设计。

### 落地价值
> 这个方案落地后，原本的工作流效率明显提高，同时推理过程更稳定、更可控，也方便企业知识沉淀和后续复用。

### 竞争力
> 目前你给到的证据是“同期产品表现优异”，这个方向是对的，但还差一个更硬的表述。建议补成具体结果后再定稿，比如：在 POC 对比中表现更好、在客户验收中通过、或者在某个核心指标上优于同期方案。

### 面试说
> 这个项目的核心，是把 Agent 的不可控问题收进结构化知识图谱和多 Agent 工作流里；我负责协同工作流、状态管理和数据库设计，最后把效率和稳定性都做出来了。
