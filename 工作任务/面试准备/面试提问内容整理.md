...
这页是你的**面试 query 页**：把高频问题、标准答案、可背诵版本都集中在这里，方便继续维护和回写。
...
| 维度 | 面试中要传达的点 |
|---|---|
| **🎯 定位** | 领域知识驱动的 ML 工程师：物理约束 → 业务规则 → 模型推理 |
| **🧩 差异点** | 不是纯调模型，而是把**领域规则写进推理流程** |
| **💡 一句话定位** | 「用ML替代物理仿真 → 用本体约束大模型推理」<br/>底层一贯：**领域知识嵌入模型推理** |

## 亚信项目：具体答案

### 项目背景
> 亚信这段项目的背景，是公司内部要自研一个 **RAG 的知识图谱增强问答系统**，主要解决 Agent / 大模型推理不可控、幻觉、上下文遗忘这些模型通病。架构参考的是 **Palantir 的本体（Ontology）架构**，再结合**渐进式披露**的思路，把企业知识结构化起来，让推理链路尽量可控、可追踪。

### 我的角色
> 我主导了多 Agent 协同工作流、状态管理和数据库设计。

### 落地价值
> 这套方案覆盖了售前、售后、数据中台和产品设计等多类业务场景：售前可以做内部数据支持和活动可行性评估，售后可以做客户满意度虚拟评估。落地后，中途人工干预减少了 70% 以上，响应时间也减少了 20%。

### 竞争力
> 这套方案不是停留在概念层，而是能在实际业务场景里跑起来，并且在同类方案对比里表现更好，说明“RAG + 知识图谱增强 + 多 Agent 协同 + 渐进式披露”的路线是可行的，而且确实有业务价值。

### 常问问题：你如何与其他团队部门协作？
> 我一般先把问题翻成大家都能理解的业务目标，再拆成各团队各自要完成的部分，比如产品定义、数据支持、研发实现和业务验证。我会把关键节点、依赖关系和交付标准提前对齐，过程中持续同步进展和风险，出现分歧就先回到目标和数据上判断，确保最后能闭环落地。

### 面试说
> 这个项目的核心，是把 Agent 的不可控问题收进 RAG 的知识图谱增强问答系统里，并采用渐进式披露来控制推理过程；我负责协同工作流、状态管理和数据库设计，最后覆盖了售前、售后、数据中台和产品设计等场景，人工干预减少了 70% 以上，响应时间也降低了 20%。
